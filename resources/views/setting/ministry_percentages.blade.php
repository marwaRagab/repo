        ...
        <div id="bs-example-modal-md" class="modal fade" tabindex="-1" aria-labelledby="bs-example-modal-md"
            ...
            <div class="modal-dialog modal-dialog-scrollable modal-lg">
                <div class="modal-content">
                    ...
                    <form id="percentageForm" action="{{ route('ministry_percentages.store') }}" method="POST"
                        enctype="multipart/form-data" onsubmit="return validatePercentageForm(event)">
                        @csrf
                    ...
                    <form id="editForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control mb-2" id="edit_name_ar"
                                    required>
                            </div>
                            <div class="form-group">
                                <label for="edit_name_en">النسبة</label>
                                <input type="text" name="percent" class="form-control mb-2" id="edit_name_en"
                                    required>
                            </div>
                        </div>
    ...
    <div class="card-body">
        <div class="table-responsive pb-4">
            <table id="all-student" class="table table-bordered border text-nowrap align-middle">
                ...
                <tbody>
                    <!-- start row -->
                    @foreach ($ministry_percentages as $item)
                        <tr>
                            <td>
                                {{ $loop->index + 1 }}
                            </td>
                            ...
                            <td>
                                <div class="action-btn">
                                    <a href="javascript:void(0)" class="text-primary edit" data-id="{{ $item->id }}"
                                        data-name_ar="{{ $item->name }}" data-name_en="{{ $item->percent }}"
...
<script>
    function validatePercentageForm(event) {
        ...
        // Validate name input (required)
        if (!nameInput.value.trim()) {
            nameError.textContent = 'الرجاء إدخال الإسم';
            isValid = false;
        }

        // Validate percent input (must be a number between 0 and 100)
        ...
        if (!percentInput.value.trim()) {
            ...
            percentError.textContent = 'النسبة يجب أن تكون رقمًا بين 0 و 100';
            isValid = false;
        }
        ...
        document.querySelectorAll('.edit').forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('editForm').setAttribute('action',
                    `/ministry_percentages/update/${id}`);
